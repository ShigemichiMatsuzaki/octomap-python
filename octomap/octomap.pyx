...
ctypedef defs.OccupancyOcTreeBase[defs.SemanticOcTreeNode].tree_iterator* semantic_tree_iterator_ptr
ctypedef defs.OccupancyOcTreeBase[defs.SemanticOcTreeNode].leaf_iterator* semantic_leaf_iterator_ptr
ctypedef defs.OccupancyOcTreeBase[defs.SemanticOcTreeNode].leaf_bbx_iterator* semantic_leaf_bbx_iterator_ptr
...
cdef class semantic_tree_iterator(semantic_iterator_base):
    ...
    def next(self):
        if self.thisptr and self.treeptr:
            if not self.__is_end():
                inc(deref(defs.static_cast[semantic_tree_iterator_ptr](self.thisptr)))
                return self
            else:
                raise StopIteration
        else:
    ...
    def isLeaf(self):
        if self.__is_acceseable():
            return defs.static_cast[semantic_tree_iterator_ptr](self.thisptr).isLeaf()
        else:
            raise NullPointerException

cdef class semantic_leaf_iterator(semantic_iterator_base):
    """
    Iterator over the complete tree (leafs).
    """
    def __cinit__(self):
    ...
    def next(self):
        if self.thisptr and self.treeptr:
            if not self.__is_end():
                inc(deref(defs.static_cast[semantic_leaf_iterator_ptr](self.thisptr)))
                return self
            else:
                raise StopIteration
        else:
...
cdef class semantic_leaf_bbx_iterator(semantic_iterator_base):
    """
    Iterator over the complete tree (leafs).
    """
    def __cinit__(self):
    ...
    def next(self):
        if self.thisptr and self.treeptr:
            if not self.__is_end():
                inc(deref(defs.static_cast[semantic_leaf_bbx_iterator_ptr](self.thisptr)))
                return self
            else:
                raise StopIteration
        else:
...
def _semantic_octree_read(filename):
    """
    Read the file header, create the appropriate class and deserialize.
    This creates a new octree which you need to delete yourself.
    """
...
cdef class SemanticOcTree:
    """
    octomap main map data structure, stores 3D occupancy grid map in an SemanticOcTree.
    """
    cdef defs.SemanticOcTree *thisptr
    cdef edt.DynamicEDTSemanticOctomap *edtptr
    def __cinit__(self, arg):
        # import numbers
        # if isinstance(arg, numbers.Number):
        self.thisptr = new defs.SemanticOcTree(<double?>arg)
        # else:
        #    self.thisptr = new defs.SemanticOcTree(string(<char*?>arg))
    ...
    def isNodeOccupied(self, node):
        if isinstance(node, SemanticOcTreeNode):
            ...
            return self.thisptr.isNodeOccupied(<defs.SemanticOcTreeNode>deref(deref((<semantic_tree_iterator>node).thisptr)))
    ...
    def isNodeAtThreshold(self, node):
        if isinstance(node, SemanticOcTreeNode):
            ...
            return self.thisptr.isNodeAtThreshold(<defs.SemanticOcTreeNode>deref(deref((<semantic_tree_iterator>node).thisptr)))
    ...
    def dynamicEDT_generate(self, maxdist,
                            ...
                            np.ndarray[DOUBLE_t, ndim=1] bbx_max,
                            treatUnknownAsOccupied=False):
        self.edtptr = new edt.DynamicEDTSemanticOctomap(<float?>maxdist,
                                                self.thisptr,
                                                defs.point3d(bbx_min[0], bbx_min[1], bbx_min[2]),
                                                defs.point3d(bbx_max[0], bbx_max[1], bbx_max[2]),
                                                <cppbool?>treatUnknownAsOccupied)
